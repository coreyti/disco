...
$.extend(Disco, {
  inherit: function(layout, template) {
    ...
    $.extend(merged_template, layout);
    $.extend(merged_template, template);
    
    merged_template.methods = {};
    $.extend(merged_template.methods, layout.methods);
    $.extend(merged_template.methods, template.methods);

    merged_template.configuration = {};
    $.extend(merged_template.configuration, layout.configuration);
    $.extend(merged_template.configuration, template.configuration);
  ...
  register_event_type: function(event_type) {
    ...
    this.prototype[event_type] = function(fn) {
      this.doc.push(new Disco.Bind(event_type, null, fn));
    };
  }
});
...
$.extend(Disco.prototype, {
  ...
  subview: function(name, template, initial_attributes) {
    this.doc.push(new Disco.OpenSubview(name))
    template.content(this, initial_attributes);
    this.doc.push(new Disco.CloseSubview(template, initial_attributes))
  },

  keyed_subview: function(name, key, template, initial_attributes) {
    this.doc.push(new Disco.OpenSubview(name, key))
    template.content(this, initial_attributes);
    this.doc.push(new Disco.CloseSubview(template, initial_attributes))
  },
  ...
  bind: function() {
    ...
    this.doc.push(new Disco.Bind(type, data, fn));
  },

//  click: function(fn) {
//    this.doc.push(new Disco.Bind('click', null, fn));
//  },
//
//  change: function(fn) {
//    this.doc.push(new Disco.Bind('change', null, fn));
//  },
...
Disco.OpenSubview = function(name, key) {
  this.name = name;
  this.key = key;
}

$.extend(Disco.OpenSubview.prototype, {
  to_string: function() {
    return "";
  },

  post_process: function(processor) {
    processor.open_subview(this.name, this.key);
  }
});

Disco.CloseSubview = function(template, initial_attributes) {
  this.template = template;
  this.initial_attributes = initial_attributes;
}

$.extend(Disco.CloseSubview.prototype, {
  to_string: function() {
    return "";
  },

  post_process: function(processor) {
    processor.close_view(this.template, this.initial_attributes);
  }
});

Disco.Bind = function(type, data, fn) {
  this.type = type;
  this.data = data;
  this.fn = fn;
}

$.extend(Disco.Bind.prototype, {
  to_string: function() {
    return "";
  },
