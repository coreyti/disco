...
Disco.Namespace = function(ns, fn_or_view) {
  if(typeof fn_or_view == 'object') {
    if( ! Disco.namespaces[ns]) {
      Disco.namespaces[ns] = {};
    }
    
    for(var key in fn_or_view) {
      Disco.namespaces[ns][key] = fn_or_view[key];
    }
  }
  else {
    if( ! Disco.namespaces[ns]) {
    ...
    if(fn_or_view instanceof Function) {
      return fn_or_view(this.namespaces[ns]);
    }
  }
};
    ...
    for(var name in dataset) {
      ...
      for(var id in dataset[name]) {
        if (!constructor.find(id)) {
          ...
          attributes.id = id;
          var object = constructors(dataset[name][id]);
...
$.extend(Disco.View, {
  ...
  register_event_type: function(event_type) {
    ...
    this.prototype[event_type] = function(fn) {
      this.doc.push(new Disco.View.Bind(event_type, null, fn));
    };
  }
});
...
$.extend(Disco.View.prototype, {
  ...
  subview: function(name, template, initial_attributes) {
    ...
    this.doc.push(new Disco.View.CloseSubview(template, initial_attributes))
  },

  keyed_subview: function(name, key, template, initial_attributes) {
    this.doc.push(new Disco.View.OpenSubview(name, key))
    template.content(this, initial_attributes);
    this.doc.push(new Disco.View.CloseSubview(template, initial_attributes))
  },
...
//
//  change: function(fn) {
//    this.doc.push(new Disco.View.Bind('change', null, fn));
//  },
...
Disco.View.OpenSubview = function(name, key) {
  this.name = name;
  this.key = key;
}
...
Disco.View.CloseSubview = function(template, initial_attributes) {
